...
\usepackage{listings}
\lstset{
  backgroundcolor=\color{black!5}, % Color de fondo
  basicstyle=\ttfamily\footnotesize,     % Fuente y tamaño
  keywordstyle=\color{purple},    % Palabras 
  commentstyle=\color{gray},      % Comentarios 
  stringstyle=\color{orange},     % Cadenas 
  numbers=left,                   % Números de línea a la izquierda
  numberstyle=\tiny\color{gray},  % Estilo de los números
  stepnumber=1,                   % Mostrar cada línea numerada
...
En este trabajo se examina en profundidad el funcionamiento de las máquinas de soporte vectorial (SVM), un algoritmo de aprendizaje automático concebido por Vladimir Vapnik y su equipo en los laboratorios de AT\&T durante la década de 1960 y presentado en su libro \textit{The nature of statistical learning theory}, en 1999.  Aunque originalmente diseñado para abordar problemas de clasificación binaria, el modelo ha evolucionado gracias a investigaciones que han extendido su alcance a la clasificación multiclase y aplicaciones en problemas de regresión.
...
La Sección 2 desarrolla el marco matemático formal del modelo SVM, comenzando con la formulación del problema de margen duro. Posteriormente, se aborda su generalización al problema de margen blando, que permite manejar datos no separables linealmente. Además, se introduce el concepto de transformación mediante funciones kernel, lo que posibilita la extensión del modelo a espacios de características de mayor dimensionalidad.

El capítulo 3 reinterpreta el problema SVM desde la perspectiva de la regularización, destacando cómo esta formulación alternativa se relaciona con el problema de margen blando. Además, se muestra la equivalencia entre ambas metodologías.

El capítulo 4 se enfoca en un análisis detallado del procedimiento conocido como envenenamiento. Esta es una técnica adversarial en la que se inyectan datos estratégicamente diseñados para comprometer la precisión del modelo. Este capítulo explora los fundamentos teóricos de este tipo de ataque.
...
Como notación, usaremos $x$ como una variable de entrada. Notemos que esta variable puede ser un vector, donde las coordenadas correspondientes del vector se representan mediante subíndices, mientras que el i-ésimo valor observado de $x$ se escribe como $x_i$. Las variables de salida  se notarán por $y$,en el caso de las variables cualitativas algunos autores usan la letra $g$ (grupo), en el presente documento usaremos la letra $y$.

Para un grupo de dos clases, el conjunto de \textit{outputs} tomará valores $\{0, 1\}$ o $\{-1, 1\}$, según sea el caso (en las máquinas de soporte vectorial, usualmente utilizamos el conjunto $\{-1, 1\}$). Dado que todos los vectores se asumen como vectores columna, la fila i-ésima de $x$ es $x_i^T$, la transpuesta del vector $x_i$.

La tarea de aprendizaje se expresa de la siguiente manera: dada una entrada representada por el vector $x$, debemos realizar una buena predicción de la salida $y$, que denotamos como $\hat{y}$ (pronunciado “y-tilde”), mediante alguna función. Si $y$ toma valores en $\mathbb{R}$, entonces $\hat{y}$ también debería hacerlo; lo mismo ocurre para salidas categóricas $g$, donde $\hat{G}$ debería tomar valores en el mismo conjunto $G$. Note que es posible transformar el grupo $G$ en un conjunto de números mediante una biyección en subconjuntos de $\mathbb{N}$, en la que a cada elemento de $G$ se le asocia un único elemento de este subconjunto.
}
    ...
    \section{Máquinas de soporte vectorial (Idea intuitiva)} {
En el presente trabajo desarrollaremos el modelo de \textbf{\textit{máquinas de soporte vectorial}} (\textbf{\textit{SVM}}, por sus siglas en inglés), el cual pertenece a la categoría de modelos de clasificación. La idea esencial de este modelo consiste en lo siguiente: dado un conjunto de \textit{inputs}, donde cada uno pertenece a una de dos categorías, se construye un modelo capaz de predecir si un nuevo \textit{input} pertenece a una categoría o a la otra. Usualmente, estos \textit{inputs} se representan como puntos en diagramas de dispersión para facilitar su visualización.
...
\end{figure} El \textit{SVM} busca un hiperplano que separe de forma óptima los puntos de una clase de los de la otra. Es importante aclarar que no en todos los conjuntos de datos es posible encontrar un hiperplano que logre una separación perfecta; sin embargo, podemos construir un modelo óptimo que minimice la cantidad de datos mal clasificados (ver Figura 1.1). En los casos en los que sí es posible encontrar un hiperplano que separe correctamente todos los datos, los denominamos casos separables. Por el contrario, si no es posible encontrar un hiperplano que separe correctamente todos los datos, los denominaremos casos no separables.
 \begin{figure}[h]
    ...
    \label{fig:imagenes}
\end{figure}
 Cabe señalar que dicho hiperplano puede no ser único por lo que buscamos el hiperplano que maximice la distancia con los puntos más cercanos a él (ver figura1 1.2), por este motivo las \textit{SVM} también son conocidas como clasificadores de margen máximo. A estos puntos que se encuentran más cerca del hiperplano, pero que a su vez máximiza la distancia se les suele llamar \textit{vectores de soporte} (hablaremos de ellos con más detalle en la Sección 2).
	...
	\section{Envenamiento (Idea intuitiva)} {El aprendizaje estadístico puede ser usado como una herramienta en la toma de decisiones en una gran variedad de problemas, incluyendo aquellos relacionados con la seguridad, pues un ejemplo de su aplicación puede ser la identificación de comportamientos irregulares. En este ámbito, se ha utilizado para la detección de spam, intrusiones o fraudes. Sin embargo, recientemente han surgido problemas relacionados con el envenenamiento de datos. Este es un tipo de ataque en el que se manipula o contamina el conjunto de datos utilizado en el entrenamiento de los modelos de aprendizaje automático, con el fin de comprometer la precisión y eficacia de los algoritmos y los procesos de toma de decisiones.
...
Los ataques contra los algoritmos de aprendizaje automático pueden clasificarse en varios tipos, pero nos centraremos principalmente en los ataques de envenenamiento, los cuales pertenecen a una categoría denominada ataques causales, en los que se inyectan puntos de datos especialmente diseñados para corromper el proceso de aprendizaje. Este tipo de ataque es particularmente relevante desde el punto de vista práctico, ya que, aunque generalmente no se tiene acceso directo a la base de datos de entrenamiento, es posible introducir nuevos datos en el conjunto de entrenamiento (por ejemplo, cuando los datos son recopilados a través de Internet), lo que brinda una oportunidad para envenenar los datos.  

En particular, nos centraremos en una familia específica de ataques de envenenamiento contra las (\textit{SVM}). Estos ataques consisten en inyectar datos en el conjunto de entrenamiento que han sido diseñados específicamente para aumentar el error de prueba del modelo en la función de pérdida.  
...
Antes de desarrollar el modelo, haremos un breve resumen de algunos resultados útiles para su construcción. Sin embargo, al ser un tema que extiende muchos conceptos, no indagaremos en los detalles, pero dejaremos referencias para consulta.
   ...
   \section{Conceptos basicos de Geometría}
   {
...
$$\hat{\beta} = (\beta_1, \beta_2, \ldots, \beta_p)$$
podemos predecir un valor de salida $Y$ mediante el modelo lineal:
$$ \hat{Y}=\hat{\beta_0}+\sum_{j=1}^{p} x_j \hat{\beta}_j$$

Donde $\beta_0$ es el Sesgo \footnote{Sesgo: En el contexto de los modelos lineales el Sesgo nos dice el valor promedio esperado para la variable de respuesta $y$,  cuando las variables predictoras $x_i$ son iguales a cero.} o también conocido como \textit{Intercepto} en el aprendizaje automático.
...
\end{equation} 
Naturalmente, si un punto satisface la ecuación anterior, entonces dicho punto está en el hiperplano. En caso de que un punto $X$ no satisfaga la ecuación \eqref{eq:1.1}, pueden ocurrir dos casos:
 \begin{equation}
	\beta_0 + \beta_1 x_1+\beta_2 x_2 + \dots +\beta_p x_p= \beta_0+\sum_{j=1}^{p} X_j \hat{\beta}_j>0
\end{equation}
 \begin{equation}
	\beta_0 + \beta_1 x_1+\beta_2 x_2 + \dots +\beta_p x_p=\beta_0+\sum_{j=1}^{p} X_j \hat{\beta}_j<0 
\end{equation}
Esto nos dice que la entrada $x$ se encuentra en algún lado del hiperplano, pero no sobre este. Por lo que podemos pensar en el hiperplano como una división del espacio $p$-dimensional en dos mitades. Para ver en qué lado del plano se encuentra una observación $X$, basta con calcular el signo de la ecuación \eqref{eq:1.1}. 
...
dado que estamos en \( \mathbb{R}^2 \), esto es una línea recta.

Para cualquier dos puntos \( x_1 \) y \( x_2 \) que se encuentren en \( L \),
\[ \beta^T (x_1 - x_2) = 0 \]
pues ambos puntos satisfacen la ecuación del hiperplano y por lo tanto el producto punto de la diferencia de estos dos puntos con el vector $\beta^T$ es cero, es decir, 
\[ \beta^* = \frac{\beta}{\|\beta\|} \]
es el vector normal unitario a la superficie de \( L \).


Para cualquier punto \( x_0 \) en \( L \),
\[ \beta^T x_0 = -\beta_0 \]

La distancia firmada (esto es el valor de las distancias pero que dependen del signo de acuerdo al lado del hiperplano que se encuentren) de cualquier punto \( x \) a \( L \) está dada por
\[ \beta^{*T} (x - x_0) = \frac{1}{\|\beta\|} (\beta^T x + \beta_0) = \frac{1}{\|f'(x)\|} f(x) \]

Esto se puede deducir de las propiedades anteriores y del hecho de que $f'(x)=\beta$. 
...
Esta propiedad nos será de gran utilidad más adelante, cuando intentemos identificar cuáles son los vectores con la menor distancia al hiperplano. 

Nuestro objetivo, entonces, es encontrar este hiperplano que separe los datos maximizando esta distancia a los puntos de cada clase. Para ello, existen diferentes métodos de construcción, los cuales desarrollaremos con más detalle en el siguiente capítulo.
    }
   
   \section{Teoria de la optimización}
   {
Para resolver un problema de Machine Learning es necesario resolver un problema de optimización, en el caso de las SVM se tiene que resolver un problema convexo con restricciones lineales, por lo que en el presente capítulo daremos un breve resumen a la teoria de optimización   

Un problema de optimización está compuesto por una función objetivo \( f(x) \) y algunas restricciones \( c_i(x) \), con \( i=1, \dots, n \). Esencialmente, el problema de optimización tiene el siguiente esquema(La función objetivo puede ser tanto para maximizar como para minimizar).
...
Una forma particular de estas transformaciones, que son convenientes desde el punto de vista computacional, es el llamado Dual Wolfe. El fin del Dual del Wolfe es relacionar las condiciones de primer orden, remplazando las condiciones de restricción por un requisito de optimalidad en la función lagrangian. R. Fletcher, (1981) describe las condiciones de primer orden, estas son conocidas comunmente como condiciones de Karush-Kuhn-Tucker (KKT).
    ...
    \begin{align}
        ...
        &\alpha \geq 0 \notag
    \end{align}
Es importante observar que el número de variables y restricciones ha cambiado, ya que, en principio teníamos $m$ variables y $n$ restricciones, ahora tenemos $n$ variables junto con $m$ restricciones de igualdad y $n$ restricciones de desigualdad. La dualización nos  ayudar a que los problemas sean más sencillos de resolver, aunque también puede ocurrir lo contrario.
...
\section{Conceptos basicos de Algebra}
{
En muchas ocasiones, no es posible construir un modelo lineal que separe los datos de manera adecuada. Por lo tanto, es necesario ir más allá de la linealidad. La idea es aumentar el vector de entradas \( X \) con más variables, que son transformaciones de \( X \), y luego usar modelos lineales en este espacio ampliado. Este enfoque permite crear modelos lineales en el espacio ampliado, aunque sean modelos no lineales en el espacio original, lo que nos ayuda proporcionando mayor flexibilidad y capacidad para capturar relaciones complejas en los datos.

Antes de continuar, daremos unas definiciones importantes descritas por Hofmann et al., 2008.

    
    Definición. Un \textbf{\textit{kernel}} es una función \( K: X \times X \to \mathbb{R} \) que mide la similitud entre dos puntos en un espacio con producto interno. La función kernel se define de tal manera que, para todos \( x, x' \in X \),
    \begin{equation}
     K(x, x') = \langle h(x), h(x')\rangle_\mathcal{H} , \label{eq: 1.9}
    \end{equation}
    donde \(\langle \cdot, \cdot \rangle\) denota el producto punto en el espacio de características (entiendase de momento como un espacio ampliado) y $h: X \to \mathcal{H} $ es una función que mapea el espacio de entrada $X+$ en un espacio de características $\mathcal{H} $ con producto escalar. El kernel permite calcular el producto punto en el espacio de características $\mathcal{H} $ sin necesidad de mapear explícitamente los puntos a ese espacio.

    Definición. Dado un kernel \( K \) y entradas \( x_1, \ldots, x_n \in X \), la matriz \( n \times n \)  $$\mathbf{K} := (k(x_i, x_j))_{ij}$$  se llama la \textit{\textbf{matriz de Gram}} (o matriz de kernel) de \( K \) con respecto a \( x_1, \ldots, x_n \).

    Definición. Sea \( X \) un conjunto no vacío. Una función \( K: X \times X \to \mathbb{R} \) que para todo \( n \in \mathbb{N} \), \( x_i \in X \), \( i =1, \dots, n \) da lugar a una matriz de Gram que cumple que 
    \begin{equation}
        \sum_{i,j=1} c_i c_j \mathbf{K}_{ij} \geq 0
    \end{equation}
    para todo $c_i \in \mathbb{R}$ es llamada  \textit{\textbf{kernel definida positiva}}.
    
    En resumen los kernels, pueden considerarse como productos punto generalizados, que nos dan una medida de similitud entre los puntos en el espacio $\mathcal{H}$. Usuamenlte los mas usados dentro del contexto de las máquinas de soporte vectorial son, el kernel polinomial, la función de base radial (RBF) y kernel sigmoidal, en el capitulo 3 daremos mas detalles de su uso.
}

\section{Espacios de Hilbert con nucleo Reproducible}{
Como se mencionó en la sección anterior, en ocasiones no es posible construir un modelo lineal en el espacio de entradas, por lo que se tiene que ampliar la dimensión a un espacio de caracteristicas para poder construir un modelo de clasificación lineal en este espacio. Los espacios de Hilbert con Kernel reproducible (RKHS) nos ayudan con esta tarea, como se verá en esta sección.

La mayoria de los modelos de Machine Learning permiten una representacion como función de regularización.

La regularización es un método que nos permite restringir el proceso de estimación para evitar un sobreajuste del modelo. Una clase general de problemas de regularización tiene el siguiente esquema:
     
     \begin{equation}
         \min_{f \in \mathcal{H}} \left[ \sum_{i=1}^{N} L(y_i, f(x_i)) + \lambda J(f) \right] \label{eq: 1.4.1}
     \end{equation}
     Donde:
     \begin{itemize}
        \item $L(y_i, f(x_i))$ es una función de perdida.
    ...
    \end{itemize}
      ...
      \begin{equation}
          f(X) = \sum_{k=1}^{N} \alpha_k \phi_k(X) + \sum_{i=1}^{M} \theta_i G(X - x_i)
      \end{equation}
...
Una subclase importante de problemas del tipo \eqref{eq: 1.4.1} son los generados por las funciones kernels \(K(x,y)\) y el correspondiente espacio de funciones \(\mathcal{H}_K\), al que se le llama \textit{Espacio de Hilbert con Kernel Reproducible}.

La teoría de los espacios de Hilbert con núcleo reproducible nos asegura que, para cada función kernel definida positiva, es posible construir su espacio de Hilbert con núcleo reproductor asociado y viceversa, Rohan Shiloh, 2007. En particular, tenemos el teorema de Moore-Aronszajn que nos dice: \textit{Todo kernel definido positivo \(K (\cdot, \cdot )\) en \(X \times X\) es un kernel reproductor para algún RKHS único de funciones en \(X\). Y a la inversa, todo RKHS tiene un kernel definido positivo asociado.} Más adelante veremos que las máquinas de soporte vectorial pueden determinar el hiperplano óptimo en este espacio asociado utilizando el llamado \textbf{truco del kernel}.

      
Sean $x,y \in \mathbb{R}^p$ y consideremos el espacio de funciones generado por \(\{K(\cdot, y) \mid y \in \mathbb{R}^p\}\), es decir, combinaciones lineales de la forma $f(x) = \sum_m \alpha_m K(x, y_m)$.
    ...
    \begin{equation}
        \min_{f \in \mathcal{H}_K} \sum_{i=1}^{N} L(y_i f(x_i)) + \lambda \|f\|^2_{\mathcal{H}_K} \label{eq: 1.4.7}
    \end{equation}
    o equivalentemente 
     \begin{equation}
        \min_{\{c_j\}_{j=1}^{\infty}} \left[ \sum_{i=1}^{N} L\left( y_i, \sum_{j=1}^{\infty} c_j \phi_j(x_i) \right) + \lambda \sum_{j=1}^{\infty} c_j^2 \gamma_j \right] \label{eq: 1.5.1}
    \end{equation}
    Se puede demostrar que la solución de \eqref{eq: 1.4.7} tiene dimensión finita y de la forma 
    \begin{equation}
        f(x) = \sum_{i=1}^{N} \alpha_i K(x, x_i)
    \end{equation}
    La función $h_i(x) = K(x, x_i)$ es una función base (Como función del primer argumento) tambien conocida como el \textit{representador de evaluación} en $x_i$ en $\mathcal{H}_K$. Se puede probar que para $f \in \mathcal{H}_K$
    $\inprod{K(\cdot, x_i)}{f }_{\mathcal{H}_K} = f(x_i)$.  Analogamente 
    $\inprod{K(\cdot, x_i)}{K(\cdot, x_j)}_{\mathcal{H}_K} = K(x_i, x_j)$ (la propiedad de reproducción de $\mathcal{H}_K$, esta propiedad nos muestra que es posible remplazar el producto de vectores en el espacio de características con una evaluación del núcleo sobre el espacio de entrada y a esto se le suele llamar truco \textit{truco del kernel} ) y por tanto 
    \begin{equation}
        J(f) = \sum_{i=1}^N \sum_{j=1}^N K(x_i, x_j) \alpha_i \alpha_j
    \end{equation}
    Para $ f(x) = \sum_{i=1}^N \alpha_i K(x, x_i)$.
    Usando estas dos ultimas ecuaciones en \eqref{eq: 1.4.7}, obtenemos el criterio de dimensión finita 
    \begin{equation}
        \min_\alpha \left[ L(y, \mathbf{K} \alpha) + \lambda \alpha^T \mathbf{K} \alpha \right]
    \end{equation}
    Donde $\mathbf{K}$ es una matriz de $N\times N$ con entradas $K(x_i,x_j)$. Asi un criterio de dimensión infinita, se reduce aun problema de dimensión finita, a esto se le denomina la propiedad del nucleo.
...
En el capítulo 3, una vez que se defina la máquina de soporte vectorial, podremos observar que el problema de clasificación de dos clases tiene la forma  
    ...
    \begin{equation}
        \min_{\alpha_0, \alpha} \left[ \sum_{i=1}^{N} \left(1 - y_i f(x_i) \right)_+ + \frac{\lambda}{2} \alpha^T \mathbf{K} \alpha \right] \label{eq: 1.7.1}
    \end{equation}
    
    Esta representación del modelo corresponde a un problema de regularización, como el descrito en la ecuación \ref{eq: 1.4.1}.  
    
    Donde $[z]_+$ denota la función de pérdida, conocida como \textit{hinge loss}, que mapea cada variable en un número real. Intuitivamente, representa el error o costo de las variables al ser clasificadas. La función de pérdida nos ayuda a determinar cómo se penalizan los errores en los problemas de clasificación y regresión, en consecuencia, afecta el proceso de optimización del modelo. 
    ...
    El uso de la función Hinge loss en SVM permite encontrar un hiperplano óptimo que maximiza el margen entre clases. Sin embargo, su naturaleza no diferenciable en ciertos puntos ha llevado al desarrollo de versiones suavizadas, como la variante de Huber, que facilitan la optimización mediante métodos de descenso de gradiente.
...
\chapter{Hiperplanos separadores}
	{Como mencionamos anteriormente, nuestro objetivo reside en buscar un hiperplano que separe correctamente a los datos. A los clasificadores que buscan una combinación lineal de las características de entrada y devuelven el signo se les llaman perceptrones (Rosenblatt, 1948), que son clasificadores lineales y el precursor de las redes neuronales. Desde el punto de vista geométrico, el perceptrón busca trazar un límite de decisión lineal en el espacio de características, con el cual poder decidir la clasificación de las observaciones. Sin embargo, Vapnik, en 1996, junto con los laboratorios de AT\&T, desarrollaron un nuevo método denominado Máquinas de Soporte Vectorial, que describiremos en esta sección. Para este problema pueden ocurrir dos casos: en el primero, el conjunto es linealmente separable, es decir, existe un hiperplano separador que clasifica correctamente a todos los datos de entrenamiento; en el segundo caso, el conjunto no es linealmente separable, por lo que se pueden permitir algunos términos clasificados en el lado incorrecto del margen. Estos son conocidos como hiperplano de margen máximo e hiperplano de margen suave, respectivamente (hard margin and soft margin), ver figura 1.1. }
	
    \section{Caso Separable. Hiperplano de Margen Maximo }
    {Dado un conjunto de entrenamiento \( (x_1, y_1, x_2, y_2, \dots, x_N, y_N) \), donde \( x_i \in \mathbb{R}^p \) \quad \text{y} \quad \( y_i \in \{-1, +1\} \), podemos construir el \textit{hiperplano de margen máximo}, el cual se construye calculando la distancia (perpendicular) desde cada observación de ambas clases a un hiperplano separador dado.

A la distancia mínima de todas las observaciones al hiperplano se le conoce como \textit{margen}.
...
\begin{figure}[h]
    ...
    \caption{Las lineas continuas muestran algunos de los posibles hiperplanos de separación, y las lineas punteadas, amarillas y verdes, muestran sus margenes, en el caso de la recta negra y sus correspendientes margen es la solución optima que maximiza el tamaño del margen, y al que llamamos hiperplano de margen máximo.}
    \label{fig:imagenes}
\end{figure}         
         
Este hiperplano que separa los datos, a veces también se le conoce como \textit{el clasificador de margen máximo}. Para saber cómo se clasifica una observación de prueba \( X^* \), nos basamos en el signo de
$$ f(x^*) = \beta_0 + \beta_1 x^*_1 + \beta_2 x^*_2 + \dots + \beta_p x^*_p = \beta_0 + \sum_{i=1}^{p} \beta_i x^{*}_i $$

donde \( \beta_0, \ldots, \beta_p \) son los coeficientes del hiperplano de margen máximo. Los puntos para los cuales la distancia del plano a estos (de tal forma que el margen sea el mayor) es la mínima se les conoce como \textit{vectores de soporte} y son vectores en un espacio \( p \)-dimensional que 'soportan' el hiperplano de margen máximo, en relación con que si estos puntos se movieran ligeramente, el hiperplano también se movería.
...
El modelo matemático del hiperplano de margen máximo se construye a continuación. Un hiperplano que separe correctamente debe satisfacer las siguientes dos condiciones:
    ...
    \begin{equation}
    	\beta_0+ x_i^T \beta \geq 0 \qquad \text{si } y_i = +1 			\label{eq: 2.1}
    \end{equation}
    
     \begin{equation}
    	\beta_0+ x_i^T \beta \leq 0 \qquad \text{si } y_i = -1
    	\label{eq: 2.2}
    \end{equation}
    Es otras palabras, cada observación la clasifica en algun lado del plano según el signo de la función.
        ...
        \begin{equation}
             y_i (\beta_0 + x_i^T \beta) \geq 0 \quad \text{para todo } i = 1, \ldots, N \label{eq: 2.3.1}
        \end{equation}
        ...
        \begin{equation}
            \frac{ x_i^T \beta + \beta_0}{\|\beta\|}
        \end{equation}
        (ver capitulo 1.4).Sea \(M\) es el margen descrito en la parte anterior. Haciendo uso de \eqref{eq: 2.3.1}, la ecuación anterior y del hecho de que cada punto debe de estar a una distancia mayor que la del margen, entonces tenemos
        \begin{equation}
        \frac{1}{\|\beta\|} (y_i  (x_i^T \beta + \beta_0))  \geq M \label{eq: 2.5}
        \end{equation}
        ...
        \begin{equation}
            \max_{\substack{\beta,\beta_0}} M \quad \text{sujeto a} \quad y_i (x_i^T \beta + \beta_0) \geq M \|\beta\| \quad \text{para} \quad i = 1, \ldots, N \label{eq: 2.3}
        \end{equation}
...
Si consideramos \( \beta \) y \( \beta_0 \) tales que satisfacen estas desigualdades, entonces cualquier múltiplo positivo también las satisface, por lo que, por convenio, se toma \( \|\beta\| = \frac{1}{M} \).

Entonces, \( M \cdot \|\beta\| = 1 \).

Por lo que nuestro problema ahora radica en minimizar \( \|\beta\| \). Con el fin de simplificar el análisis, elevamos la función al cuadrado y multiplicamos por \( \frac{1}{2} \), obteniendo así una función convexa. Esta transformación no modifica la posición del mínimo de la función, así que la ecuación \eqref{eq: 2.3} es equivalente a:

        
        \begin{equation}
            \min_{\beta, \beta_0} \frac{1}{2} \|\beta\|^2 \quad \text{sujeto a} \quad y_i (x_i^T \beta + \beta_0) \geq 1 \quad \text{para } i = 1, \ldots, N \label{eq: 2.4}
        \end{equation}
        
        
         Este es un problema de optimización cuadrática, con restricciones de desigualdad lineal, (ver capitulo 1.5).  El cual tiene una forma dual si la función a optimizar y las restricciones son estrictamente convexas, en este caso resolver el problema dual es equivalente a obtener la solución al problema primal (se puede mostrar que tanto la función objetivo como las restricciones son funciones estrictamente convexas).
            
        Haciendo uso de la función de Lagrange Primal con el objetivo de minimizar con respecto a \(\beta\) y \(\beta_0\),
        \begin{align}
            L_p &= \frac{1}{2} \|\beta\|^2 - \sum_{i=1}^{N} \alpha_i (y_i (x_i^T \beta + \beta_0) - 1)  \label{eq:Ecuacion de lagrange}
            \\
            &=\frac{1}{2} \inprod{\beta}{\beta}  - \sum_{i=1}^{N} \alpha_iy_i(x_i^T \beta) - \sum_{i=1}^{N} \alpha_i y_i \beta_0 + \sum_{i=1}^{N}\alpha_i \notag
        \end{align}
        Derivando parcialmente respecto de \(\beta\) y \(\beta_0\) respectivamente tenemos
        \begin{align}
            \frac{\partial L_p}{\partial \beta} &= \frac{1}{2} (\beta\cdot 1 + 1\cdot\beta) - \sum_{i=1}^{N} \alpha_i y_i x_i \notag \\
        ...
        \begin{align}
            \frac{1}{2} \|\beta\|^2 - \sum_{i=1}^{N} \alpha_i (y_i (x_i^T \beta + \beta_0) - 1) &= \frac{1}{2} \inprod{\sum_{i=1}^{N} \alpha_i y_i x_i}{\sum_{i=1}^{N} \alpha_i y_i x_i} - \sum_{i=1}^{N} \alpha_iy_i(x_i^T \sum_{k=1}^{N} \alpha_k y_k x_k) \notag\\
        &-\sum_{i=1}^{N} \alpha_i y_i \beta_0 + \sum_{i=1}^{N}\alpha_i \notag \\
           &= \frac{1}{2} \sum_{i=1}^{N} \alpha_iy_ix_i^T \left( \sum_{k=1}^{N} \alpha_k y_k x_k \right)\notag\\
           &  - \sum_{i=1}^{N} \alpha_iy_i(x_i^T \sum_{k=1}^{N} \alpha_k y_k x_k) + \sum_{i=1}^{N} \alpha_i \notag\\
           &= \sum_{i=1}^{N} \alpha_i- \frac{1}{2} \sum_{i=1}^{N}\sum_{k=1}^{N} \alpha_iy_ix_i^T   \alpha_k y_k x_k   
        \end{align}
    
        Por lo que tenemos el siguiente problema 
        \begin{equation}
             L_D=  \sum_{i=1}^{N} \alpha_i- \frac{1}{2} \sum_{i=1}^{N}\sum_{k=1}^{N} \alpha_iy_ix_i^T   \alpha_k y_k x_k  \label{eq:Problema Dual} 
        \end{equation}
...
Por las condiciones de Karush–Kuhn–Tucker,
         ...
         \begin{equation}
            \alpha_i \left[ y_i \left( x_i^T \beta + \beta_0 \right) - 1 \right] = 0 \quad \forall i \label{eq: 2.12}
        \end{equation}
        De esto notemos que si $\alpha_i>0 $ entonces $y_i\left( x_i^T \beta + \beta_0 \right)=1$ esto nos indica que $x_i$ esta en el espacio de separación (se encuentra en los extremos del margen), por la condición en la ecuación \eqref{eq: 2.4}. A estos puntos en el extremo del margen les llamamos \textit{vectores de soporte}.
\begin{figure}[h]
    \centering
...
Por otra parte, si \( y_i \left( x_i^T \beta + \beta_0 \right) - 1 > 0 \), entonces \( x_i \) no está en el espacio de separación, es decir, está a una distancia mayor que los extremos del margen, por lo tanto, en la condición \eqref{eq: 2.12}, \( \alpha_i = 0 \).

Por otra parte, observemos que podemos calcular el vector \( \beta \) en la expresión \eqref{eq:Beta}, ya que se define únicamente en términos de una combinación lineal de los vectores de soporte, es decir, aquellos puntos para los cuales \( \alpha_i \) es mayor que cero.
...
Por lo tanto, el plano de separación óptimo produce una función \( \hat{f}(x) = x^T \hat{\beta} + \hat{\beta}_0 \) para clasificar nuevas observaciones,
$$ \hat{G}(x) = \text{sign} \hat{f}(x) $$ 
que depende únicamente de los vectores de soporte (pues \( \beta \) y \( \beta_0 \) dependen únicamente de ellos).
...
Para ilustrar mejor el funcionamiento de las máquinas de soporte vectorial, consideremos un ejemplo sencillo: supongamos un conjunto de entrenamiento de dos puntos \( A \) y \( C \), sean \( y_A \) y \( y_C \) sus respectivas etiquetas, entonces tenemos el problema
        ...
        \begin{equation}
            \min_{\beta, \beta_0} \frac{1}{2} \|\beta\|^2 \quad \text{sujeto a} \quad y_A (A^T \beta + \beta_0) = 1, \\
            \quad y_C (C^T \beta + \beta_0) = 1  \label{eq: 2.2.1}
        \end{equation}
...
Definamos la función de lagrange como 
    \begin{equation*}
        L_p=\frac{1}{2} \|\beta\|^2 - \alpha_A(y_A (A^T \beta + \beta_0))- \alpha_C(y_C (C^T \beta + \beta_0))
    \end{equation*}
Podemos optimizar esta función mediante el metodo de lagrange. Sin perdida de generalidad supongamos que $y_{A}=1$ y $y_{C}=-1$
    \begin{equation*}
        L_p=\frac{1}{2} \|\beta\|^2 - \alpha_A(A^T \beta + \beta_0)+\alpha_C (C^T \beta + \beta_0)
    \end{equation*}
Entonces derivando parcialmente 
    \begin{align*}
        \frac{\partial L_p}{\partial \beta} &= \frac{1}{2} (\beta\cdot 1 + 1\cdot\beta) - \alpha_AA^T  +\alpha_C C^T\\
        &=\beta - \alpha_AA^T  +\alpha_C C^T =0
    \end{align*}
de donde 
    \begin{equation}
        \beta =\alpha_AA^T - \alpha_C C^T \label{eq: 2.2.2}
    \end{equation}
Por otra parte 
    \begin{align*}
...
De esto ultimo obtenemos que  $\alpha_A  = \alpha_C$. Entonces sustituyendo en \eqref{eq: 2.2.2}, obtenemos 
    \begin{equation}
        \beta =\alpha_A(A^T - C^T) \label{eq: 2.2.3}
    \end{equation}
y de las restricciones de \eqref{eq: 2.2.1} podemos obtener $(A^T \beta + \beta_0) = 1,  \quad (C^T \beta + \beta_0) = -1$, restando estas dos ecuaciones obtenemos 
    \begin{equation}
        (A^T - C^T)\beta =2 \label{eq: 2.2.4}
    \end{equation}
    
luego multiplicando \eqref{eq: 2.2.3}, por $(A^T - C^T)$ 
    \begin{equation*}
        (A^T - C^T) \beta =(A^T - C^T)\alpha_A(A^T - C^T)=\alpha_A(A^T - C^T)^2
    \end{equation*}
...
Por lo que 
    \begin{equation*}
        \alpha_A=\alpha_C=\frac{2}{(A^T - C^T)^2}
    \end{equation*}
Y en consecuencia 
    \begin{align*}
        \beta &=2\frac{(A^T - C^T)}{(A^T - C^T)^2} \\
        \beta_0 &=1-A^T\beta=1-2 A^T \frac{(A^T - C^T)}{(A^T - C^T)^2}=\frac{-(A^T)^2 +(C^T)^2}{(A^T - C^T)^2}
    \end{align*}

       Así, si tenemos dos puntos \( A \) y \( C \), podemos obtener los coeficientes del hiperplano a partir de \( A \) y \( C \).

Cabe destacar que, aunque ninguna de las observaciones de entrenamiento caiga en el margen, por la forma en la que está construido el modelo, para las observaciones de prueba no necesariamente se cumple esto, por lo que un margen amplio en los datos de entrenamiento llevará a una buena separación de los datos de prueba.
...
Notamos que usamos fuertemente el hecho de que se cumpla la condición \eqref{eq: 2.1} y \eqref{eq: 2.2}, es decir, que los datos sean separables. Cuando los datos no son separables, no hay una solución factible para este problema usando el modelo anterior. Por lo tanto, haremos uso de un criterio alternativo al \eqref{eq: 2.1} y \eqref{eq: 2.2}.
    ...
    \section{Hiperplano de Margen Maximo Caso No Separables }
    { Supongamos ahora que las clases no son separables, es decir, que para algunas observaciones no se cumple \eqref{eq: 2.3.1}, por lo que hay observaciones en el lado incorrecto del plano separador. Nuestro objetivo ahora es medir qué tanto estas variables están violando la restricción y minimizar esta violación de la restricción.

Para ello, definamos las variables de holgura \( \xi = (\xi_1, \xi_2, \ldots, \xi_N) \), con \( \xi_i \geq 0 \). Podemos modificar las restricciones en \eqref{eq: 2.5} para obtener la restricción
\begin{equation}
...
\end{figure}          
        ...
        Como en \eqref{eq: 2.4}  podemos reescribir el problema considerando $M=\frac{1}{\| \beta \|}$, como:
     
        \begin{equation}
            \min \| \beta \| \quad \text{sujeto a} \quad y_i (x_i^T \beta + \beta_0) \geq 1 - \xi_i \quad \forall i                                                             \quad \xi_i \geq 0 \quad \sum_{i=1}^{N}\xi_i \leq C
        \end{equation}
        ...
        \begin{equation}
            \min_{\beta, \beta_0} \quad \frac{1}{2} \|\beta\|^2 + C \sum_{i=1}^{N} \xi_i \quad \text{sujeto a} \quad \xi_i \geq 0 \quad \text{y} \quad y_i (x_i^T \beta + \beta_0) \geq 1 - \xi_i \quad \forall i \label{eq: caso no separable funcion a minimizar}
        \end{equation}
...
Así, el nuevo problema de optimización consistirá en encontrar el hiperplano de separación definido por \( \beta \) y \( \beta_0 \), que minimice \eqref{eq: caso no separable función a minimizar}. Al hiperplano resultante se le denomina hiperplano de separación de margen blando (Soft margin). La manera de obtener el hiperplano de separación es análoga al caso separable, por lo que los pasos siguientes son similares a los de la sección anterior.

        
        La función de Lagrange primal es:
        \begin{align}
            L_P &= \frac{1}{2} \|\beta\|^2 + C \sum_{i=1}^{N} \xi_i - \sum_{i=1}^{N} \alpha_i \left[ y_i (x_i^T \beta + \beta_0) - (1 - \xi_i) \right] - \sum_{i=1}^{N} \mu_i \xi_i \label{eq: 2.19} \\
            &=\frac{1}{2} \inprod{\beta}{\beta} + C \sum_{i=1}^{N} \xi_i - \sum_{i=1}^{N} \alpha_i  y_i x_i^T \beta - \sum_{i=1}^{N} \alpha_i  y_i\beta_0 + \sum_{i=1}^{N}\alpha_i(1 - \xi_i)  - \sum_{i=1}^{N} \mu_i \xi_i \notag
        \end{align} 
        ...
        \begin{align}
            \frac{\partial L_p}{\partial \beta} &= \frac{\partial}{\partial \beta} \left[ \frac{1}{2} \inprod{\beta}{\beta} + C \sum_{i=1}^{N} \xi_i - \sum_{i=1}^{N} \alpha_i  y_i x_i^T \beta - \sum_{i=1}^{N} \alpha_i  y_i\beta_0 + \sum_{i=1}^{N}\alpha_i(1 - \xi_i)  - \sum_{i=1}^{N} \mu_i \xi_i \right] \notag\\
            &= \beta- \sum_{i=1}^{N} \alpha_i  y_i x_i^T \label{eq: parcial beta}
        \end{align} 
        \begin{align}
            \frac{\partial L_p}{\partial \beta_0} &= \frac{\partial}{\partial \beta_0} \left[ \frac{1}{2} \inprod{\beta}{\beta} + C \sum_{i=1}^{N} \xi_i - \sum_{i=1}^{N} \alpha_i  y_i x_i^T \beta - \sum_{i=1}^{N} \alpha_i  y_i\beta_0 + \sum_{i=1}^{N}\alpha_i(1 - \xi_i)  - \sum_{i=1}^{N} \mu_i \xi_i \right] \notag\\
            &= -\sum_{i=1}^{N} \alpha_i  y_i \label{eq: parcial beta cero}
        \end{align}
         \begin{align}
            \frac{\partial L_p}{\partial\xi_i} &= \frac{\partial}{\partial \xi_i} \left[ \frac{1}{2} \inprod{\beta}{\beta} + C \sum_{i=1}^{N} \xi_i - \sum_{i=1}^{N} \alpha_i  y_i x_i^T \beta - \sum_{i=1}^{N} \alpha_i  y_i\beta_0 + \sum_{i=1}^{N}\alpha_i -\sum_{i=1}^{N}\alpha_i \xi_i  - \sum_{i=1}^{N} \mu_i \xi_i \right] \notag\\
            &= C-\alpha_i-\mu_i \label{eq: parcial xi i}
        \end{align}
    
        Igualando a cero las ecuaciones \ref{eq: parcial beta}-\ref{eq: parcial xi i} obtenemos
        \begin{align}
            \beta= \sum_{i=1}^{N} \alpha_i  y_i x_i^T \label{eq:beta ns}
        \end{align} 
        \begin{align}
        ...
        \begin{align}
            L_D = & \frac{1}{2} \inprod{\sum_{i=1}^{N} \alpha_i  y_i x_i}{\sum_{k=1}^{N} \alpha_k  y_k x_k^T} +  \sum_{i=1}^{N} C\xi_i - \sum_{i=1}^{N} \alpha_i  y_i x_i \sum_{i=1}^{N} \alpha_k  y_k x_k^T \notag\\
            &- \beta_0 \sum_{i=1}^{N} \alpha_i  y_i + \sum_{i=1}^{N}\alpha_i(1 - \xi_i)  - \sum_{i=1}^{N} \mu_i \xi_i \notag\\
            &= \frac{1}{2} \sum_{i=1}^{N}\sum_{k=1}^{N} \alpha_i  y_i x_i  \alpha_k  y_k x_k^T
            +\sum_{i=1}^{N} (\alpha_i+\mu_i)\xi_i-\sum_{i=1}^{N}\sum_{i=1}^{N} \alpha_k \alpha_i  y_i x_i   y_k x_k^T +  \sum_{i=1}^{N}\alpha_i \notag \\
            & - \sum_{i=1}^{N}\alpha_i \xi_i - \sum_{i=1}^{N}\mu\xi_i \notag\\
            &= \sum_{i=1}^{N}\alpha_i-\frac{1}{2} \sum_{i=1}^{N}\sum_{k=1}^{N} \alpha_i  y_i x_i  \alpha_k  y_k x_k^T \label{eq: 2.26}
        \end{align}
        ...
        \begin{align}
            \alpha_i \left[ y_i \left( x_i^T \beta + \beta_0 \right) - \left( 1 - \xi_i \right) \right] &= 0 \label{eq:kkt 1}\\
            \mu_i \xi_i &= 0 \label{eq:kkt 2}\\
            y_i \left( x_i^T \beta + \beta_0 \right) - \left( 1 - \xi_i \right) &\geq 0 \label{eq:kkt 3}
        \end{align}
        
De la ecuación \ref{eq:C ns}, si tomamos \( \alpha_i = 0 \), entonces \( C = \mu_i \), por lo que en la ecuación \ref{eq:kkt 2} se deduce que \( \xi_i = 0 \). Se concluye entonces que para las entradas \( x_i \) cuyo \( \alpha_i = 0 \), corresponden a casos separables.

En los casos no separables, \( x_i \) se caracteriza por \( \xi_i > 0 \), por lo que nuevamente de \eqref{eq:kkt 2}, \( \mu_i = 0 \) y por \ref{eq:C ns} se concluye que \( C = \alpha_i \). Así, cuando \( C = \alpha_i \), corresponde a casos no separables.
...
$$ y_i \left( x_i^T \beta + \beta_0 \right) - 1 = 0. $$
Es decir, \( x_i \) está en la frontera del margen. Y por lo tanto, \( x_i \) es un vector de soporte si y solo si \( 0 < \alpha_i < C \).

        
        Notemos que de la ecuación \eqref{eq:beta ns}:
        \begin{align}
            \hat{\beta}= \sum_{i=1}^{N} \hat{\alpha_i}  y_i x_i^T  \label{eq:beta ns}
        \end{align}
        $\hat{\beta}$ es una combinación lineal de los $\hat{\alpha_i}$ no nulos, para los cuales $x_i$ cumple las restricciones \eqref{eq:kkt 1} - \eqref{eq:kkt 3}, en otras palabras $\hat{\beta}$ queda determinado únicamente por los vectores de  soporte y de las observaciones para los cuales $\alpha_i=C$.
        ...
        Maximizar el dual es un problema de programación cuadrática convexa, mas facil de resolver que el primal.
    
        Una vez obtenidas $\beta_0$ y $\beta$, podemos escribir la función de decisión dada por 
        \begin{equation}
            \hat{G}(x) = \operatorname{sign}\left[\hat{f}(x)\right] = \operatorname{sign}\left[x^T \hat{\beta} + \hat{\beta}_0\right]
        \end{equation}
...
\begin{figure}[h]
    ...
    \caption{No hay forma de separar los datos usando un hiperplano lineal, por lo que se amplia el espacio de entrada, para construir un modelo lineal en el espacio de caracteristicas, en este caso se ha usado el kernel base radial.}
    \label{fig:imagenes}
\end{figure}     



     Sea $h(x_i):X \rightarrow \mathcal{H}$ una función que mapea nuestro espacio de entradas al espacio de características. Donde $h(x_i) = (h_1(x_i), h_2(x_i), \ldots, h_M(x_i))$ para $i = 1, \ldots, N$ y \(h_m(x_i)\) para \(m = 1, \ldots, M\) estas últimas son llamadas las funciones base.
     Entonces podemos representar el problema de optimización \eqref{eq: 2.19} a traves de los vectores de entrada y la función $h(x_i)$ mediante el producto interno.
     Por lo que la función dual de lagrange \eqref{eq: 2.26} en el espacio de 		 caracteristicas es 
     \begin{equation}
         L_D = \sum_{i=1}^{N} \alpha_i - \frac{1}{2} \sum_{i=1}^{N} \sum_{i' = 1}^{N} \alpha_i \alpha_{i'} y_i y_{i'}  \inprod{h(x_i)}{h(x_{i'})}
     \end{equation}
     Y la solución  $f(x)$ puede reescribirse como:
     \begin{equation}
         f(x) = h(x)^T \beta + \beta_0 = \sum_{i=1}^{N} \hat{\alpha_i} y_i  \inprod{h(x)}{h(x_{i})} + \beta_0 \label{eq: 2.3.1}
     \end{equation}
     
     Sin embargo para realizar esto se requiere el cálculo de del producto interno $\inprod{h(x_i)}{h(x_{i'})$ en un espacio de alta dimensión, afortunadamente el uso de un kernel definido positivo $K$ puede reducir significativamente estos cálculos complejos
     
     
     de modo que  $K(x, x') = \inprod{h(x)}{h(x')} $ por lo que podemos reescribir \eqref{eq: 2.3.1} como
     $$f(x) = \sum_{i=1}^{N} \hat{\alpha}_i y_i K(x, x_i) + \hat{\beta}_0$$ 
...
\begin{itemize}
    \item \textit{Kernel polinomico de grado d}     
            \begin{equation}
                K(x, x') = (1 + \inprod{x}{x'})^d \label{eq: 2.38}
            \end{equation}
    ...
    \item \textit{Kernel de redes neuronales}
            \begin{equation}
                K(x, x') = \tanh(\kappa_1 \inprod{x}{x'} + \kappa_2) 
            \end{equation}
...
\end{itemize}
    ...
    Con $\kappa_1 > 0$ y $\kappa_2 \in \mathbb{R}$ Una vez establecido el kernel el problema se reduce  a encontrar los $\hat{\alpha_i}$ que optimizan el problema dual:
     \begin{equation}
         \max \sum_{i=1}^{N} \alpha_i - \frac{1}{2} \sum_{i=1}^{N} \sum_{i' = 1}^{N} \alpha_i \alpha_{i'} y_i y_{i'}  K(x_i, x_i') \label{eq: 2.41}
     \end{equation}
     Sujeto a 
     \begin{equation}
 ...
 Así, la función de decisión lineal vendrá expresada como solución del problema de optimización anterior, en términos del kernel \( K \) y el costo \( C \). Es importante destacar que, como en las secciones anteriores, la solución al problema dual se puede representar únicamente mediante los vectores de soporte, por lo que este problema no depende de la dimensionalidad del espacio de características, sino solo de la cardinalidad del conjunto de vectores de soporte.
    ...
    \begin{equation}
    	1= y_i f(x_i) =y_i \left( \sum_{j=1}^{N} \alpha_j y_j K(x_i, x_i') + 			\beta_0 \right) 
    \end{equation}
...
\begin{equation}
    K(x,x')= \inprod{h(x)}{h(x')}=\left[ 1 + \inprod{x}{x'} \right]^2 \label{eq: 3.11}
\end{equation}
...
\begin{align*}
    \inprod{h(x)}{h(x')} &= \left[ 1 + \inprod{x}{x'} \right]^2 = \left[ 1 + \inprod{(x_1,x_2)}{(x'_1,x'_2)} \right]^2  \\
...
\chapter{Penalización y Nucleos Reproducibles}
	\section{SVM como metodo de penalización}{
En algunas ocasiones, nos es conveniente ver el SVM como un modelo de penalización, donde el objetivo es minimizar el error en la clasificación de los datos de entrenamiento e intentar controlar la complejidad del modelo. Este enfoque nos permite penalizar al modelo cuando realiza una mala clasificación y también nos ayuda a evitar el sobreajuste.

En este sentido, podemos ver el problema como


\begin{equation}
    \min_{\beta,\beta_0} \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_+ + \frac{\lambda}{2} \|\beta\|^2 
\end{equation}

Pues recordemos que el problema original viene dado por  
        \begin{equation}
            \min_{\beta, \beta_0} \quad \frac{1}{2} \|\beta\|^2 + C \sum_{i=1}^{N} \xi_i \quad \text{sujeto a} \quad \xi_i \geq 0 \quad \text{y} \quad y_i (x_i^T \beta + \beta_0) \geq 1 - \xi_i \quad \forall i \label{eq: 3.10}
        \end{equation}
Entonces de la restricción
    \begin{align*}
        y_i f(x_i) &= y_i (x_i^T \beta + \beta_0) \geq 1 - \xi_i  \\
        \Rightarrow \xi_i &\geq 1-y_i (x_i^T \beta + \beta_0) 
    \end{align*} 
Podemos definir $\xi_i = \max(0, 1 - y_i f(\mathbf{x}_i)) = \left[1 - y_i f(x_i)\right]_+$, esta usualmente es llamada la función hinge loss como se menciona en la seección 1.7 (es natural definir la función de esa forma, pues $\xi_i$ nos indica el error; y para los puntos correctamente clasificados no estamos considerando un error, por lo que el valor en la función es 0. Contrario a esto, para aquellos puntos que sí están mal clasificados, la función toma un valor mayor que 0), por lo que, sustituyendo en \eqref{eq: 3.10}, se obtiene
        ...
        \begin{equation}
            \min_{\beta, \beta_0} \quad \frac{1}{2} \|\beta\|^2 + C \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_+ \quad \text{sujeto a} \quad \left[1 - y_i f(x_i)\right]_+ \geq 0 
        \end{equation}
Ahora si consideramos $C=\frac{1}{\lambda}$ 
        \begin{align}
            &\min_{\beta, \beta_0} \quad \frac{1}{2} \|\beta\|^2 + \frac{1}{\lambda} \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_+  \notag \\
            & \Rightarrow \min_{\beta, \beta_0} \quad \frac{\lambda}{2} \|\beta\|^2 + \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_+ \label{eq: 3.12}
        \end{align}


Donde el término \(\left[1 - y_i f(x_i)\right]_+\) es la función hinge loss (Pérdida de bisagra), y \([\,]_+\) indica la parte positiva; note que podemos eliminar la restricción, pues por la naturaleza de la función esta siempre sera positiva; más explícitamente esta función está definida por 
    \begin{equation}
        \ell^{\text{hinge}}(f(x), y) = 
        \begin{cases}
        0 & \text{si } yf(x) \geq 1 \\
...
Ahora notemos que, para puntos fuera del margen, es decir, aquellos para los cuales se cumple que $y_i f(x_i) > 1$, no tienen contribución a la función. Para los vectores de soporte, es decir, para aquellos para los cuales $y_i f(x_i) = 1$, la función tampoco contribuye a la pérdida. Finalmente, para los puntos dentro del margen se cumple que $y_i f(x_i) < 1$, y por lo tanto su contribución a la función Hinge loss es $1 - y_i f(x_i)$.

Y la segunda parte es un término de penalización \(\frac{\lambda}{2} \|\beta\|^2\) que castiga a los modelos más complejos. Aquí el parámetro \(\lambda\) sirve como controlador de la magnitud de la penalización aplicada a los coeficientes del modelo \(\beta\). De cierta forma, determina el equilibrio entre aumentar el tamaño del margen y asegurarse de que $x_i$ esté en el lado correcto del margen. Aunque la ecuación \eqref{eq: 3.12} no es diferenciable, podemos encontrar su punto mínimo mediante el método de descenso del gradiente (Oscar Ramos, 2020).
...
Al hacer que los coeficientes de la expansión lineal \(f(x) = \beta_0 + h(x)^T \beta\) (excluyendo \(\beta_0\)) se reduzcan hacia cero, imponemos una penalización sobre la magnitud de los coeficientes de \(\beta\), lo que nos ayuda a evitar un posible sobreajuste y a simplificar el modelo. Cuando el parámetro de regularización \(\lambda\) es muy grande, la penalización es mayor, lo que restringe la magnitud de los coeficientes \(\beta\) y lleva a un modelo más simple, posiblemente con un margen más pequeño. Mientras que si \(\lambda\) tiende a cero, la penalización se vuelve muy pequeña y el modelo se enfoca más en minimizar la función de pérdida sin preocuparse por la complejidad del modelo. Cuando los datos son separables y $\lambda \rightarrow 0$, la penalización en los coeficientes se reduce, lo que genera un hiperplano que maximiza el margen.
...
Ahora nos centraremos en describir el SVM en términos de estimación de funciones en espacios de Hilbert con núcleos reproductores (RKHS, por sus siglas en inglés). La ventaja es que ofrece una forma estructurada de trabajar con funciones en un espacio posiblemente infinito, gracias a la propiedad de reproducción del núcleo. Esta parte sustenta formalmente la sección 3 del capítulo 2, por lo que puede ser omitido en un principio.
   ...
   Supongamos una base h, que surge de la expansión en eigenvalores (Valores propios) de un kernel definido positivo K 
    ...
    \end{equation}
    y $\ h_m(x) = \sqrt{\delta_m} \phi_m(x)$ Si hacemos $\theta_m = \sqrt{\delta_m} \ \beta_m$ podemos escribir la función de penalización como 
    \begin{align}
        \min_{\beta,\beta_0} \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_+ + \frac{\lambda}{2} \|\beta\|^2 &= \min_{\beta_0,\theta} \ \sum_{i=1}^{\infty} \left[1 - y_i \left(\beta_0 + \sum_{m=1}^{\infty} \theta_m \phi_m(x_i)\right)\right]_+ + \frac{\lambda}{2} \sum_{m=1}^{\infty} \frac{\theta_m^2}{\delta_m}
    \end{align}
    ...
    \begin{equation}
        f(x) =\beta_0 + \sum_{i=1}^{N} \alpha_i K(x, x_i) \label{eq: 3.1.3}
    \end{equation}
    En particular hay una versión equivalente al criterio de optimización \eqref{eq: 2.26} dado por 
    \begin{equation}
        \min_{\beta_0, \alpha} \sum_{i=1}^{N} \left(1 - y_i f(x_i)\right)_{+} + \frac{\lambda}{2} \alpha^T \mathbf{K} \alpha
    \end{equation}
Donde $\mathbf{K}$ es una matriz de $N \times N$ donde se evalúan todos los pares de características de entrada en el kernel.  

De la ecuación \eqref{eq: 3.1.3} podemos ver que el hiperplano separador está dado por las evaluaciones de las observaciones el nucleo K y este a su vez esta determinado por los productos internos de las funciones base $h_i$.

   
   Más generalmente podemos expresar el problema como 
   \begin{equation}
       \min_{f \in \mathcal{H}} \sum_{i=1}^{N} \left[1 - y_i f(x_i)\right]_{+} + \lambda J(f)
   \end{equation}
donde $\mathcal{H}$ es el espacio estructurado de funciones, y $J(f)$ es un regularizador adecuado en ese espacio. Por ejemplo, podríamos usar el espacio de funciones aditivas, por lo que el kernel vendría dado por $K(x, x') = \sum_{j=1}^{p} K_j(x_j, x_j')$, y cada \(K_j\) es el kernel apropiado para el spline suavizado univariado en \(x_j\) (para más detalles, consultar Wahba, 1990). 
...
\begin{table}[h]
    ...
    \begin{tabular}{c|ccccccc}
        \multicolumn{8}{c}{\textbf{Kernel Polinomial:} \( k(x, x') = \left( \frac{\langle x, x' \rangle}{256} \right)^d \)} \\ \hline
        \textit{d} & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\ \hline
        Tasa de error\% & 8.9 & 4.7 & 4.0 & 4.2 & 4.5 & 4.5 & 4.7 \\
...
\begin{table}[h]
    ...
    \setlength{\tabcolsep}{10pt}
    \begin{tabular}{c|ccccccc}
        \multicolumn{8}{c}{\textbf{Kernel RBF:} \( k(x, x') = \exp \left( - \gamma \| x - x' \|^2 / (256) \right) \)} \\ \hline
        \textit{ $\gamma$ } & 0.25 & 0.5 & 0.85 & 1.25 & 2 & 5 & 10 \\ \hline
        Tasa de error\% & 5.3 & 5.0 & 4.9 & 4.3 & 4.4 & 4.4 & 4.5 \\
...
\begin{table}[h]
    ...
    \setlength{\tabcolsep}{10pt}
    \begin{tabular}{c|ccccccc}
        \multicolumn{8}{c}{\textbf{Kernel de red neuronal:} \( k(x, x') = \tanh \left( 2 \langle x, x' \rangle/ 256 + \kappa_2 \right) \)} \\ \hline
...
Note que, con cualquier kernel y una selección adecuada de parámetros, se puede alcanzar un porcentaje de error del 4\%. Esto resalta la importancia de un ajuste adecuado de los parámetros, con el fin de mejorar el desempeño del modelo.
...
Una vez establecido cómo funciona el modelo SVM, nos enfocaremos en todos los conceptos relacionados con el envenenamiento de las Máquinas de Soporte Vectorial. Como mencionamos en el capítulo 1, existen diferentes métodos para atacar el rendimiento de un modelo de machine learning, y es importante que un modelo sea robusto ante tales situaciones, pues podría haber involucrado información importante.

Supongamos que tenemos un conjunto de datos de entrenamiento $D_{tr}=\{ (x_i, y_i) \}_{i=1}^{n}$. Denotemos por $Q = yy^{T} \odot \textbf{K}$ a la matriz de etiquetas de \(K\), donde \(\textbf{K}\) es la matriz de evaluación del kernel (en este contexto, $\odot$ indica el producto entrada a entrada de las matrices). Notemos que \(Q_{ij} = y_i y_j K(x_i, x_j)\) es simétrica y definida positiva, pues \(K\) es un kernel definido positivo.

Dependiendo del valor de \(\alpha_i\), podemos clasificar los puntos de entrenamiento en tres conjuntos:

    
    \begin{align*}
        & S = \{x_i \mid 0 < \alpha_i < C\} \qquad \text{Vectores de soporte de margen}\\
        & E = \{x_i \mid \alpha_i = C\} \qquad \text{Vectores de error de soporte}\\
        & R = \{x_i \mid \alpha_i = 0 \} \qquad \text{Puntos de reserva}
    \end{align*} 
    
Usaremos las letras minúsculas \(s\), \(e\), \(r\) para indexar los puntos correspondientes de vectores o matrices. Por ejemplo, \(Q_{ss}\) denota la submatriz \(Q\) correspondiente a los vectores de soporte del margen.

Asumiendo el rol del atacante, nuestro objetivo es encontrar un punto \((x_c, y_c)\) que, al ser añadido al conjunto de entrenamiento, disminuya la precisión del clasificador SVM. La elección de la etiqueta del punto de ataque \(y_c\) es arbitraria pero fija. Nos referimos a la clase de esta etiqueta como clase atacante y a la otra como clase atacada.

    
    Procedemos primero extrayendo un conjunto de datos de validación $D_{val}= \{ (x_i,y_i)\}_{i=1}^{m}$ nuestro objetivo es maximizar la pérdida hinge que el SVM comete sobre los puntos en el conjunto de validación, donde el SVM se entrena con $D_{tr} \cup \{(x_c,y_c)\}$.
    \begin{equation}
        \max_{x_c} L(x_c)=\sum_{k=1}^{m}(1-y_k f_{x_c}(x_k))_+ =\sum_{k=1}^{m} (-g_k)_+ \label{eq: 4.0}
    \end{equation}
    ...
    \begin{align}
        g_k &= -(1-y_k f_{x_c} (x_k)) \notag\\
        &= -1+y_k (\sum_{j} \alpha_j y_j k(x_k,x_j)+b) \notag\\
        &= -1+ \sum_{j } \alpha_j y_k y_j k(x_k,x_j)+y_k b \notag\\
        &= \sum_{j \} Q_{kj} \alpha_j + y_k b - 1  \label{eq: 4.1} \\
        &= \sum_{j \neq c} Q_{kj} \alpha_j(x_c) + Q_{kc}(x_c) \alpha_c(x_c) + y_k b(x_c) - 1. \notag
    \end{align} 
    
    
    
Note que la función \(L(x_c)\) es una función objetivo no convexa, pero puede optimizarse mediante el método de ascenso del gradiente.

Este método consiste en escoger inicialmente un punto de ataque \(x_{c}^{(0)}\) y, después, actualizar este punto de ataque mediante \(x_{c}^{(p)} = x_{c}^{(p-1)} + tu\), donde \(p\) es el número de iteración, \(u\) es un vector de norma uno que representa la dirección del ataque y \(t\) es el tamaño del paso que se da en cada iteración. La dirección del ataque de \(u\) debe alinearse con el gradiente de la función \(L\) con respecto a \(u\), y esta debe calcularse en cada iteración.

Aunque la función hinge loss no es diferenciable en todos sus puntos debido a que es una función definida por partes, podemos evitar este problema centrándonos únicamente en los puntos \(k\) que tienen una contribución no nula a \(L\). En otras palabras, consideramos sólo los puntos \((x_k, y_k)\) que están mal clasificados, aquellos para los cuales \(-g_k > 0\).

Las contribuciones que hacen estos puntos al gradiente pueden calcularse diferenciando \eqref{eq: 4.1} con respecto a \(u\), lo que nos ayuda a ver cómo cambia \(L\) en función de la dirección de \(u\).

    
    
    \begin{align*}
        \frac{\partial g_k}{\partial u} &= \sum_{j \in S} \frac{\partial} {\partial u} (Q_{kj} \alpha_j )  + y_k \frac{\partial b}{\partial u} \\          
        &= \sum_j Q_{kj} \frac{\partial \alpha_j} {\partial u}  + \sum_j   \alpha_j  \frac{\partial Q_{kj}} {\partial u}   + y_k \frac{\partial b}{\partial u} \\
        &=Q_{k1}\left[ \frac{\partial \alpha_1}{\partial u_1},  \dots, \frac{\partial \alpha_1}{\partial u_d} \right]+Q_{k2}\left[ \frac{\partial \alpha_2}{\partial u_1},  \dots, \frac{\partial \alpha_2}{\partial u_d} \right]+ \dots + Q_{ks}\left[ \frac{\partial \alpha_s}{\partial u_1},  \dots, \frac{\partial \alpha_s}{\partial u_d} \right]+ \\
         & \left[ \frac{\partial Q_{k1}}{\partial u_1},  \dots, \frac{\partial Q_{k1}}{\partial u_d} \right] \alpha_1  +  \left[ \frac{\partial Q_{k2}}{\partial u_1},  \dots, \frac{\partial Q_{k2}}{\partial u_d} \right]\alpha_2+ \dots + \left[ \frac{\partial Q_{ks}}{\partial u_1},  \dots, \frac{\partial Q_{ks}}{\partial u_d} \right]\alpha_s + y_k \frac{\partial b}{\partial u} \\ 
        &=\left[ Q_{k1} \frac{\partial \alpha_1}{\partial u_1}+ \dots + Q_{ks} \frac{\partial \alpha_s}{\partial u_1}, Q_{k1} \frac{\partial \alpha_1}{\partial u_2}+ \dots + Q_{ks} \frac{\partial \alpha_s}{\partial u_2}, \dots,  Q_{k1} \frac{\partial \alpha_1}{\partial u_d}+ \dots + Q_{ks} \frac{\partial \alpha_s}{\partial u_d}   \right] +\\
        &\left[  \frac{\partial Q_{k1}}{\partial u_1}\alpha_1 + \dots +  \frac{\partial Q_{ks} }{\partial u_1} \alpha_s, Q_{k1} \frac{\partial Q_{k1}}{\partial u_2}\alpha_1 + \dots +  \frac{\partial Q_{ks} }{\partial u_2} \alpha_s, \dots,  \frac{\partial Q_{k1}}{\partial u_d}\alpha_1 + \dots +  \frac{\partial Q_{ks} }{\partial u_d} \alpha_s \right] + y_k \frac{\partial b}{\partial u} \\
        &=Q_{ks} \frac{\partial \alpha}{\partial u} + \frac{\partial Q_{kc}}{\partial u} \alpha_c + y_k \frac{\partial b}{\partial u}
    \end{align*}
    Por lo tanto 
    \begin{equation}
        \frac{\partial g_k}{\partial u} =Q_{ks} \frac{\partial \alpha}{\partial u} + \frac{\partial Q_{kc}}{\partial u} \alpha_c + y_k \frac{\partial b}{\partial u} \label{eq: 4.2}
    \end{equation}
    ...
    $$
        ...
        \end{bmatrix}, \quad \frac{\partial Q_{kc}}{\partial u}=\left[ \frac{\partial Q_{kc}}{\partial u_1},  \dots, \frac{\partial Q_{kc}}{\partial u_d} \right], \quad \frac{\partial b}{\partial u}=\left[ \frac{\partial b}{\partial u_1},  \dots, \frac{\partial b}{\partial u_d} \right].
    $$
...
Para refinar más las expresiones para el gradiente es importante destacar que, para el punto \(i\)-ésimo en el conjunto de entrenamiento, las condiciones de KKT para la solución óptima pueden expresarse como
    ...
    \begin{align*}
        \frac{\partial b}{\partial u} &= -\frac{1}{\zeta} \alpha_c \mathbf{v}^\top \cdot \frac{\partial Q_{sc}}{\partial u} \\
        \frac{\partial \alpha}{\partial u} &= -\frac{1}{\zeta} \alpha_c \left( \zeta Q_{ss}^{-1} - \mathbf{v}\mathbf{v}^{\top}\right) \cdot \frac{\partial Q_{sc}}{\partial u} 
    \end{align*}
    Una vez calculadas estas expresiones podemos sustituir las en la ecuación \eqref{eq: 4.2}
    \begin{align*}
        \frac{\partial g_k}{\partial u} &=Q_{ks} \frac{\partial \alpha}{\partial u} + \frac{\partial Q_{kc}}{\partial u} \alpha_c + y_k \frac{\partial b}{\partial u} \\
         &=- Q_{ks} \frac{1}{\zeta} \alpha_c \left( \zeta Q_{ss}^{-1} - \mathbf{v}\mathbf{v}^{\top}\right) \cdot \frac{\partial Q_{sc}}{\partial u}  - y_k \frac{1}{\zeta} \alpha_c \mathbf{v}^\top \cdot \frac{\partial Q_{sc}}{\partial u} + \frac{\partial Q_{kc}}{\partial u} \alpha_c \\
    \end{align*}
    Tomando $M_k=-\frac{1}{\zeta} \left(    Q_{ks}\left( \zeta Q_{ss}^{-1} - \mathbf{v}\mathbf{v}^{\top}\right) +y_k\mathbf{v}^{\top}\right)$ obtenemos 
    \begin{align*}
        \frac{\partial g_k}{\partial u} &= M_k\frac{\partial Q_{sc}}{\partial u} \alpha_c + \frac{\partial Q_{kc}}{\partial u} \alpha_c  =  \left(M_k\frac{\partial Q_{sc}}{\partial u}  + \frac{\partial Q_{kc}}{\partial u}\right) \alpha_c \\
    \end{align*}    
   Sustituyendo esta expresión en \eqref{eq: 4.0} obtenemos el gradiente deseado que se usa para optimizar nuestro ataque
   \begin{equation}
       \frac{\partial L}{\partial u} = \sum_{k=1}^{m} \frac{\partial g_k}{\partial u} =\sum_{k=1}^{m} \left\{ M_k \frac{\partial Q_{sc}}{\partial u} + \frac{\partial Q_{kc}}{\partial u} \right\} \alpha_{c} \label{eq: 4.7}
   \end{equation}
...
\begin{itemize}
    ...
    \[
    \frac{\partial K_{ic}}{\partial u} = \frac{\partial e^{-\frac{\gamma}{2} \| x_i - x_c^{(p)} \|^2}}{\partial u} = K(x_i, x_c^{(p)}) \gamma t (x_i - x_c^{(p)})
    \]
\end{itemize}
La dependencia de \(x_c^{(p)}\) y, por lo tanto, de \(u\) en los gradientes de los kernels no lineales puede evitarse sustituyendo \(x_c^{(p)}\) con su valor en la iteración anterior \(x_c^{(p-1)}\), siempre que el tamaño de paso \(t\) sea lo suficientemente pequeño. Esto nos asegura que, en cada paso de ajuste, asumimos que el cambio es mínimo, y así, podemos hacer una aproximación en los cálculos sin una pérdida significativa de precisión.

Como mencionamos anteriormente, el algoritmo depende del proceso iterativo del descenso del gradiente y se ajusta en cada paso siguiendo la dirección del gradiente. Este proceso comienza eligiendo un punto inicial \(x_c^{(0)}\) de la clase objetivo a la que se quiere atacar (clase atacada). Este punto es simplemente una copia de un punto ya existente en el conjunto de entrenamiento, pero se le invierte la etiqueta de clase. Es importante que el punto esté lo suficientemente dentro de la clase atacada y que no esté muy cerca de la frontera, ya que un punto demasiado cercano a la frontera puede hacer que se convierta en uno de reserva, lo que detendría el progreso.

Posteriormente, con el punto de ataque inicializado, se entrena una SVM usando el conjunto de datos de entrenamiento \(D_{tr}\) sin el punto de ataque \(x_c^{(0)}\), obteniendo así los parámetros \(\alpha_i\) y el sesgo \(b_0\).
...
\begin{algorithm}
\caption{Poisoning attack against SVM}
    \begin{algorithmic}[1]
    \REQUIRE Datos de entrenamiento $\mathcal{D}_{\text{tr}}$; Datos de validación $\mathcal{D}_{\text{val}}$; $y_c$ la etiqueta de clase del punto de ataque; punto de ataque inicial $x_c^{(0)}$; y el tamaño del paso $t$.
    \ENSURE $x_c$, el punto final de ataque.
    \STATE $\{\alpha_i, b\} \leftarrow$ entrenar el SVM con$\mathcal{D}_{\text{tr}}$.
    \STATE $k \leftarrow 0$.
    \REPEAT
        \STATE Re-calcular la solución del SVM con el conjunto $\mathcal{D}_{\text{tr}} \cup \{(x_c^{(p)}, y_c)\}$ usando SVM incremental (e.g., \textit{Cauwenberghs \& Poggio, 2001}). Para este paso ocupamos $\{\alpha_i, b\}$.
        \STATE Calcular $\frac{\partial L}{\partial u}$ con $\mathcal{D}_{\text{val}}$ de acuerdo a la ecuación \eqref{eq: 4.7}.
        \STATE Establecer $u$ un vector unitario alineado con $\frac{\partial L}{\partial u}$.
        \STATE $k \leftarrow k + 1$ y $x_c^{(p)} \leftarrow x_c^{(p-1)} + t u$.
    \UNTIL $L\left(x_c^{(p)}\right) - L\left(x_c^{(p-1)}\right) < \varepsilon$.
    \RETURN $x_c = x_c^{(p)}$.
    \end{algorithmic}
\end{algorithm}
...
Una vez entrenado nuestro modelo usamos los datos de $X\_test$, para checar la precisión del modelo.

Podemos calcular la precisión de nuestro modelo con los datos de prueba con la métrica \textit{accuracy} que se define como:
\begin{align*}
...
\begin{lstlisting}[language=Python]
    ...
    print("Precision antes del envenenamiento:{:.2%}".format(accuracy))
\end{lstlisting}
\begin{figure}[h]
...
\begin{lstlisting}[language=Python]
    ...
    for ax, images, label, digito in zip(axes, X_train, 
        ...
        ax.imshow(images, cmap=plt.cm.gray_r, interpolation='nearest')
        ax.set_title('Digito\npredicho: ' + str(label), loc = "left")
        ax.set_title('Digito\nreal: ' + str(digito), loc = "right")
\end{lstlisting} 
\begin{figure}[h]
...
\begin{lstlisting}[language=Python]
    ...
    print("Precision despues de 1 instancia de envenenamiento: 
    {:.2%}".format(accuracy))
\end{lstlisting}
\begin{figure}[h]
            \includegraphics[scale=1]{19.png}
\end{figure}
Observemos que, con un único punto de envenenamiento, no logramos degradar el rendimiento del modelo. Para maximizar el impacto, podríamos incrementar el número de puntos envenenados, lo que reduciría aún más la precisión del SVM.
...
\begin{lstlisting}[language=Python]
    print(f'Dimension del conjunto de entrenamiento:{X_train.shape[0]}')
    print(f'Dimension del conjunto de prueba:{X_test.shape[0]}')
    print(f'Dimension del conjunto de validacion:{X_val.shape[0]}')
\end{lstlisting}
...
Posteriormente obtenemos la precisión antes del ataque
    \begin{lstlisting}[language=Python]
        y_pred = svm.predict(X_test)
        accuracy_before_attack = accuracy_score(y_test, y_pred)
        print("Precision antes del ataque: 
...
Nuevamente entrenamos otro SVM con los datos envenenados y obtenemos su precisión.
\begin{lstlisting}[language=Python]
	svm_clf2 = SVC(kernel='linear', C=1.0, random_state=42)
	...
	print("Precisión despues de 1 punto de ataque: {:.2%}".format(accuracy2))
\end{lstlisting}
\begin{figure}[h]
...
\begin{lstlisting}[language=Python]
	...
	y_pred3 = svm_clf3.predict(X_test)
	accuracy3 = accuracy_score(y_test, y_pred3)
	print("Precisión despues de 5 puntos de ataque:
	{:.2%}".format(accuracy3))
\end{lstlisting}
\begin{figure}[!]
    \includegraphics[scale=0.8]{45.png}
\end{figure}
...
\begin{lstlisting}[language=Python]					
	...
	print("Precisión despues de 25 punto de ataque:
	{:.2%}".format(accuracy4))
\end{lstlisting}
\begin{figure}[!]
    \includegraphics[scale=0.8]{47.png}
\end{figure}
...
\begin{figure}[!]
    ...
    \begin{subfigure}{0.6\textwidth}
        ...
        \caption{Hiperplano despues de 5 puntos de envenenamiento, los coeficientes que definen este hiperplano son $\beta_0=3.25937224$, $\beta_1=-0.000217515019$ y $\beta_2=-0.740648859$}
    \end{subfigure}
    \hfill
    % Segunda imagen
    \begin{subfigure}{0.6\textwidth}
        \includegraphics[width=\linewidth]{51.png} 
        \caption{Hiperplano  despues de 25 puntos de envenenamiento, los coeficientes que definen este hiperplano son $\beta_0=1.78647242$, $\beta_1=-0.8989058$ y $\beta_2=0.04275622$}
    \end{subfigure}
\end{figure}
...
Los ingenieros y científicos de datos deben tener siempre presente la amenaza latente de adversarios que buscan comprometer los sistemas, explotando vulnerabilidades en las etapas de vida de un modelo de machine learning.
...
Este segundo enfoque consiste en diseñar una única función que entrene  las k SVMs binarias simultáneamente y maximice los márgenes de  cada clase, la idea es similar del método uno contra todos. Se construyen K modelos de 2 clases donde la función m-ésima separa los vectores de entrenamiento de la clase $m$ de los otros vectores por lo tanto hay k funciones de decisión, la formulación del problema es 
        ...
        Donde $\xi_{m,i}$ es el valor $m,i$ de la matriz de variables de holgura, $\phi$ es la función que mapea al espacio de caracteristicas y C es el parametro de penalización o Costo. 
