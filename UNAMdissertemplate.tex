...
Un problema de optimización restringida está compuesto por una función objetivo \( f(x) \) y algunas restricciones \( c_i(x) \), con \( i=1, \dots, n \). Esencialmente, el problema de optimización tiene el siguiente esquema(La función objetivo puede ser tanto para maximizar como para minimizar).
...
\end{figure}     
     ...
     Sin embargo para realizar esto se requiere el cálculo de del producto interno $\inprod{h(\mathbf{x}_i)}{h(\mathbf{x}_{i'})$ en un espacio de alta dimensión, afortunadamente el uso de un kernel definido positivo $K$ puede reducir significativamente estos cálculos complejos
